...
def test_summary_without_api_key_returns_401(client):
    """Request without API key is rejected."""
    test_client, _ = client
    response = test_client.post("/summary", json=VALID_PAYLOAD)
    assert response.status_code == 401


def test_summary_with_wrong_api_key_returns_401(client):
    """Request with wrong API key is rejected."""
    test_client, _ = client
    response = test_client.post("/summary", json=VALID_PAYLOAD, headers={"X-API-Key": "wrong"})
    assert response.status_code == 401


# --- Happy path ---

def test_summary_returns_200_with_valid_payload(client):
...
def test_summary_llm_response_is_returned_as_message(client):
    """The LLM response text is returned in the message field."""
    test_client, mock_service = client
    mock_service.make_analyse_request.return_value = "Custom summary output"
    response = test_client.post("/summary", json=VALID_PAYLOAD, headers=HEADERS)
    assert response.json()["message"] == "Custom summary output"


def test_summary_ollama_called_with_correct_analysis_type(client):
    """OllamaService is called with analysis_type='summary'."""
    test_client, mock_service = client
    test_client.post("/summary", json=VALID_PAYLOAD, headers=HEADERS)
    mock_service.make_analyse_request.assert_called_once()
    call_kwargs = mock_service.make_analyse_request.call_args
    assert call_kwargs.kwargs["analysis_type"] == "summary"


def test_summary_multiple_datasets_are_joined_with_separator(client):
    ...
    test_client.post("/summary", json=payload, headers=HEADERS)
    call_kwargs = mock_service.make_analyse_request.call_args
...
def test_summary_validation_returns_400(client, payload_override, description):
    """Invalid payload ({description}) is rejected with 400."""
    test_client, _ = client
    response = test_client.post("/summary", json={**VALID_PAYLOAD, **payload_override}, headers=HEADERS)
    assert response.status_code == 400


# --- Ollama errors ---

def test_summary_returns_502_on_ollama_error(client):
